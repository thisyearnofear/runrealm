...
export const DashboardScreen: React.FC = () => {
  ...
  // Use singleton pattern - get the instance once
  const dashboardService = UserDashboardService.getInstance();

  useEffect(() => {
    let isMounted = true;
    let handleDataUpdate: ((data: any) => void) | null = null;

    const initializeAndLoad = async () => {
      try {
        // Initialize the service if not already initialized
        if (!dashboardService.getIsInitialized()) {
  ...
  return (
    ...
    >
      ...
      {dashboardData?.userStats && (
        <View style={styles.section}>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statCard}>
              <Text style={styles.statNumber}>
                {dashboardData.userStats.territoriesOwned}
              </Text>
      ...
      {/* Current Run */}
      {dashboardData?.currentRun &&
        dashboardData.currentRun.status !== "completed" && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
            ...
            <View style={styles.activityItem}>
              ...
              <View style={styles.activityContent}>
                ...
                <Text style={styles.activityDescription}>
                  {formatDistance(
                    dashboardData.recentActivity.lastRun.totalDistance
                  )}{" "}
                  in{" "}
                  {formatDuration(
                    dashboardData.recentActivity.lastRun.totalDuration
                  )}
                </Text>
              </View>
            </View>
            ...
            dashboardData.recentActivity.recentAchievements.length > 0 && (
              <View style={styles.activityItem}>
                ...
                <View style={styles.activityContent}>
                  ...
                  <Text style={styles.activityDescription}>
                    {
                      dashboardData.recentActivity.recentAchievements.slice(
                        -1
                      )[0]
                    }
                  </Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionSubtitle}>
              {dashboardData.territories.length} owned
            </Text>
          </View>
              ...
              <View style={styles.territoriesSummary}>
                <View style={styles.summaryItem}>
                  <Text style={styles.summaryNumber}>
                    {dashboardData.territories.length}
                  </Text>
                ...
                <View style={styles.summaryItem}>
                  <Text style={styles.summaryNumber}>
                    {dashboardData.territories
                      .reduce(
                        (sum: number, t: any) => sum + (t.estimatedReward || 0),
                        0
                      )
                      .toFixed(2)}
                  </Text>
                ...
                {dashboardData.territories.slice(0, 3).map((territory: any) => (
                  <View
                    ...
                    style={[
                      ...
                      styles[territory.rarity || "common"],
                    ]}
                  >
            ...
            <View style={styles.walletRow}>
              ...
              >
                {dashboardData.walletInfo.address
                  ? `${dashboardData.walletInfo.address.substring(
                      0,
                      6
                    )}...${dashboardData.walletInfo.address.substring(
                      dashboardData.walletInfo.address.length - 4
                    )}`
                  : "Not connected"}
              </Text>
            </View>
            ...
            <View style={styles.insightItem}>
              ...
              <View style={styles.insightContent}>
                ...
                <Text style={styles.insightDescription}>
                  {(
                    dashboardData.aiInsights.suggestedRoute.distance / 1000
                  ).toFixed(1)}
                  km route available
                </Text>
              </View>
