...
import { View, Text, StyleSheet, Button, PermissionsAndroid, Platform, NativeEventEmitter, NativeModules, } from 'react-native';
...
const GPSTrackingComponent = ({ onRunStart, onRunStop }) => {
    ...
    useEffect(() => {
        ...
        const locationListener = GeolocationEventEmitter.addListener('Geolocation', (position) => {
            if (isTracking) {
                setLocation({
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.controlsContainer}>
        {!isTracking ? (<Button title="Start Run" onPress={handleStartRun} color="#4CAF50"/>) : (<View style={styles.runningControls}>
            <Button title="Pause" onPress={handlePauseRun} color="#FF9800"/>
            <Button title="Stop Run" onPress={handleStopRun} color="#F44336"/>
          </View>)}
        
        {isTracking && !isTracking && (<Button title="Resume" onPress={handleResumeRun} color="#2196F3"/>)}
      </View>
    </View>);
};
...
//# sourceMappingURL=GPSTrackingComponent.js.map