...
export class TerritoryDashboard {
  ...
  private renderPlayerStats(): string {
    return `
      <div class="stats-grid">
        <div class="stat-item">
          <div class="stat-value">${this.playerStats.realmBalance.toFixed(
            2
          )}</div>
        ...
        <div class="stat-item">
          <div class="stat-value">${(
            this.playerStats.totalDistance / 1000
          ).toFixed(1)}km</div>
    ...
    return this.territories
      ...
      <div class="territory-card ${territory.rarity || "common"}">
        ...
        <div class="territory-info">
          ...
          <div class="territory-stats">
            <div class="territory-score">
              <span class="score-value">${this.calculateTerritoryValue(
                territory
              )}</span>
        ...
        <div class="territory-actions">
          <button class="analyze-btn" data-geohash="${
            territory.geohash
          }">Analyze</button>
          <button class="visit-btn" data-geohash="${
            territory.geohash
          }">Visit</button>
        </div>
      </div>
  ...
  private setupInteractions(): void {
    ...
    const aiRouteBtn = this.container.querySelector(
      "#ai-route-btn"
    ) as HTMLButtonElement;
    if (aiRouteBtn) {
    ...
    const analysisBtn = this.container.querySelector(
      "#territory-analysis-btn"
    ) as HTMLButtonElement;
    if (analysisBtn) {
      analysisBtn.addEventListener("click", () =>
        this.requestTerritoryAnalysis()
      );
    }
  ...
  private requestTerritoryAnalysis(): void {
    if (this.territories.length === 0) {
      this.updateAIInsights(
        "No territories to analyze. Claim your first territory by going for a run!"
      );
      return;
    }

    this.updateAIInsights(
      "Analyzing your territories... This may take a moment."
    );
  ...
  private async analyzeTerritoryDetail(geohash: string): Promise<void> {
    try {
      ...
      this.updateAIInsights(
        "Failed to analyze territory. Please try again later."
      );
  ...
  private handleTerritoryUpdate(data: any): void {
    ...
    if (data.territory) {
      const existingIndex = this.territories.findIndex(
        (t) => t.id === data.territory.id
      );
