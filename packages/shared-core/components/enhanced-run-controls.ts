...
export class EnhancedRunControls extends BaseService {
  ...
  private createWidget(): void {
    ...
    if (widgetSystem) {
      ...
      this.subscribe('widget:toggled', (data: { widgetId: string }) => {
  ...
  private addLapToList(lap: { number: number; distance: number; time: number }): void {
  ...
  private async checkGPS(): Promise<void> {
    try {
      ...
      this.showFeedback(
        `❌ GPS check failed: ${error instanceof Error ? error.message : String(error)}`,
        'error'
      );
    }
  }
