...
export class TerritoryService extends BaseService {
    constructor() {
        super();
        this.claimedTerritories = new Map();
        this.nearbyTerritories = [];
        this.proximityThreshold = 100; // meters
        // New: Territory intent management
        this.territoryIntents = new Map();
        this.INTENT_EXPIRY_HOURS = 24; // Territory intents expire after 24 hours
    }
    async onInitialize() {
    ...
    setupEventListeners() {
        ...
        this.subscribe("web3:crossChainTerritoryClaimed", (data) => {
            this.handleCrossChainClaimConfirmation(data);
        });
        // Listen for cross-chain claim failures
        this.subscribe("web3:crossChainTerritoryClaimFailed", (data) => {
     ...
     */
    async createTerritoryIntent(bounds, plannedRoute, estimatedDistance, estimatedDuration) {
        const geohash = this.generateGeohash(bounds);
        // Create a mock RunSession for metadata generation
        const mockRunSession = {
            id: "",
            startTime: Date.now(),
     ...
     */
    async getTerritoryPreview(bounds) {
        const geohash = this.generateGeohash(bounds);
     ...
     */
    async checkIntentFulfillment(runSession) {
     ...
     */
    runOverlapsWithIntent(runSession, intent) {
     ...
     */
    cancelTerritoryIntent(intentId) {
     ...
     */
    async handleCrossChainClaimConfirmation(data) {
        try {
     ...
     */
    async handleCrossChainClaimFailure(data) {
        try {
     ...
     */
    async createTerritoryFromRun(run) {
     ...
     * Calculate territory bounds from run points
     */
    calculateTerritoryBounds(points) {
     ...
     */
    async generateTerritoryMetadata(run, bounds) {
     ...
     */
    calculateReward(difficulty, rarity) {
     ...
     */
    async identifyLandmarks(bounds) {
     ...
     */
    generateTerritoryName(bounds, landmarks) {
     ...
     */
    generateTerritoryDescription(run, difficulty, landmarks) {
     ...
     */
    async validateTerritoryUniqueness(geohash, bounds) {
     ...
     */
    territoriesOverlap(bounds1, bounds2) {
     ...
     */
    async checkTerritoryExistsOnChain(geohash) {
     ...
     */
    async claimTerritoryFromExternalActivity(runSession) {
        if (!runSession.territoryEligible || !runSession.geohash) {
            return {
                success: false,
     ...
     */
    async claimTerritory(territory) {
        try {
     ...
     */
    async checkTerritoryAvailability(geohash) {
        try {
            const contractService = this.getService("ContractService");
            if (!contractService || !contractService.isReady()) {
     ...
     */
    updateNearbyTerritories(locationInfo) {
        const nearby = [];
        for (const [id, territory] of this.claimedTerritories) {
    ...
    showProximityAlert(territory, distance) {
     ...
     */
    calculateDistanceToTerritory(location, territory) {
     ...
     */
    async loadClaimedTerritories() {
        try {
            const stored = localStorage.getItem("runrealm_claimed_territories");
            if (stored) {
                const territories = JSON.parse(stored);
