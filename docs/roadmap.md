...
| 8 | Tests & CI | 🟡 Partial (Sep 2026) | `sync:check` drift gate added to `ci.yml` (Node 20, `web` workspace name fixed); `sync-game-rules.mjs` restored. Still pending: unit tests for `ConfidentialTerritoryService`, `canSteal()`, and ghost rubber-banding. |
