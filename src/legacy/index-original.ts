...
// Optional secrets import with fallback to env or localStorage
let mbk: string | undefined;
try {
  // This file is intentionally git-ignored and may not exist locally
  const secrets = require('./appsettings.secrets') as { MAPBOX_ACCESS_TOKEN?: string };
  mbk = secrets.MAPBOX_ACCESS_TOKEN;
} catch {}
import { AnimationService } from './animation-service';
import { NextSegmentService } from './next-segment-service';
import { PreferenceService } from './preference-service';
import { EnhancedToastService } from './enhanced-toast';
import { MobileOptimizations } from './mobile-optimizations';
import { MobileUIComponents } from './mobile-ui-components';
...
if (!mbk) {
  const envToken = (process as any).env && (process as any).env.MAPBOX_ACCESS_TOKEN;
  mbk = envToken || localStorage.getItem('MAPBOX_ACCESS_TOKEN') || undefined;
}

const initialFocus = preferenceService.getLastOrDefaultFocus();
const mapStyle = getStyleById(preferenceService.getMapStyle());
if (!mbk) {
  console.error('Mapbox access token not found. Provide it via src/appsettings.secrets.ts, environment (MAPBOX_ACCESS_TOKEN), or localStorage.');
}
const mbkConst = mbk as string;
(mapboxgl as any)[atob('YWNjZXNzVG9rZW4=')] = mbkConst;
let map = new Map({
...
let nextSegmentService = new NextSegmentService(mbkConst);

let currentRun: CurrentRun = undefined;

let animationService = new AnimationService(map);
const enhancedToast = new EnhancedToastService();
const mobileOptimizations = new MobileOptimizations();
const mobileUI = new MobileUIComponents();

// Initialize mobile onboarding after everything is loaded
map.on('load', () => {
  // ... existing map load code ...
  
  // Initialize mobile onboarding
  setTimeout(() => {
    new MobileOnboarding();
  }, 1000);
});
...
function initializeEnhancedUX() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
      e.preventDefault();
      const removeButton = document.getElementById('remove-last');
      if (removeButton && !removeButton.classList.contains('slide-out')) {
        removeButton.click();
        enhancedToast.info('Last point removed');
      }
    }
  ...
  // Enhanced distance display with animation
  const originalUpdateLength = updateLengthElement;
  (window as any).updateLengthElement = function() {
    const lengthElement = document.getElementById('run-length');
    const oldValue = lengthElement?.textContent;
    
    originalUpdateLength();
    
    const newValue = lengthElement?.textContent;
    if (oldValue !== newValue && lengthElement) {
      // Add pulse animation
      lengthElement.style.animation = 'none';
      setTimeout(() => {
        lengthElement.style.animation = 'pulse 0.5s ease-out';
      }, 10);
    }
  };
  ...
  if ('vibrate' in navigator) {
    document.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).matches?.('button, .map-overlay')) {
  ...
  // Welcome message
  setTimeout(() => {
    enhancedToast.success('Welcome to RunMap! Click on the map to start planning your route.', 5000);
  }, 1000);
}

// Initialize enhanced UX after DOM is ready
