...
export class UIService {
    ...
    showToast(message, options = {}) {
        ...
        if (options.contextual && this.contextualMessages[message]) {
            const messages = this.contextualMessages[message];
            message = messages[Math.floor(Math.random() * messages.length)];
        }
        if (!this.toastContainer)
            return;
        const { type = "info", duration = 5000, showProgress = false, celebration = false, haptic = false, sound = false, action } = options;
     ...
     */
    showContextualSuccess(context, data) {
     ...
     */
    showContextualError(context, originalError) {
    ...
    getContextualMessage(context) {
        const messages = this.contextualMessages[context] || [
            "⏳ Working on it...",
    ...
    getSuccessMessage(context, data) {
        ...
        return successMessages[context] || "✅ Success! Operation completed successfully!";
    }
    getErrorMessage(context, originalError) {
        const errorMessages = {
            aiServiceDown: {
