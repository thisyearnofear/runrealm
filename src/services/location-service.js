...
export class LocationService extends BaseService {
    ...
    async onInitialize() {
        ...
        this.geocodingService = new (await import("../geocoding-service")).GeocodingService(this.config.getConfig().mapbox.accessToken);
    ...
    async searchLocations(query) {
        try {
            ...
            return results.map((result) => ({
                ...
                lat: result.center[1],
                lng: result.center[0],
                country: result.context?.find((c) => c.id.includes("country"))
                    ?.text,
    ...
    showLocationModal() {
        ...
        setTimeout(() => {
            const firstFocusable = this.locationModal.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
            if (firstFocusable) {
    ...
    setCurrentLocation(locationInfo) {
        ...
        try {
            const animationService = window.RunRealm?.services?.animation;
    ...
    setupEventHandlers() {
        ...
        // Search input
        this.domService.delegate(document.body, "#location-search-input", "input", this.debounce(async (event) => {
        ...
        document.addEventListener("keydown", (event) => {
            ...
            if (this.locationModal && this.locationModal.style.display === "flex") {
                const focusableElements = this.locationModal.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
