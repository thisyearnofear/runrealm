...
export class LocationService extends BaseService {
  ...
  protected async onInitialize(): Promise<void> {
    ...
    this.safeEmit('service:initialized', { service: 'LocationService', success: true });
  }
        ...
        async (position) => {
          ...
          try {
            const address = await this.geocodingService.reverseGeocode(
              [locationInfo.lng, locationInfo.lat]
            );
            if (address) {
            ...
            <div class="current-location-info" id="current-location-info">
              ${this.currentLocation ? `
                <div class="location-display">
                  <strong>Current:</strong> ${this.currentLocation.address || `${this.currentLocation.lat.toFixed(4)}, ${this.currentLocation.lng.toFixed(4)}`}
  ...
  private setupEventHandlers(): void {
    ...
    // Search input
    this.domService.delegate(document.body, '#location-search-input', 'input', 
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (event.key === 'Escape' && this.locationModal && this.locationModal.style.display === 'flex') {
        this.hideLocationModal();
      }
  ...
  private displaySearchResults(results: LocationSearchResult[]): void {
    ...
    resultsContainer.innerHTML = results.map(result => `
      <div class="search-result-item" data-lat="${result.lat}" data-lng="${result.lng}" data-name="${result.name}">
        ...
        ${result.region || result.country ? `<div class="result-details">${[result.region, result.country].filter(Boolean).join(', ')}</div>` : ''}
      </div>
    `).join('');
