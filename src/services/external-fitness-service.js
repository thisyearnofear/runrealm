...
export class ExternalFitnessService extends BaseService {
    constructor() {
        ...
        this.stravaRateLimiter = RateLimiterFactory.getStravaLimiter();
     ...
     */
    storeTokens(source, accessToken, refreshToken, expiresAt) {
     ...
     */
    async refreshAccessToken(source, refreshToken) {
        if (source !== 'strava') {
     ...
     */
    async getStravaActivities(page = 1, limit = 10) {
        try {
            // Apply rate limiting before making API call
            await this.stravaRateLimiter.consume(1);
            const token = await this.ensureValidToken('strava');
            const response = await fetch(`https://www.strava.com/api/v3/athlete/activities?page=${page}&per_page=${limit}`, {
                headers: {
