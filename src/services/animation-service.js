...
export class AnimationService extends BaseService {
     ...
     */
    setAIRoute(coordinates, style, metadata) {
        if (!this.map) {
     ...
     */
    async animateRouteDrawing(coordinates, style, metadata) {
        if (!this.map)
            return;
        ...
        try {
            ...
            const animate = (currentTime) => {
                ...
                if (this.map && this.map.getSource('ai-route-source')) {
                    ...
                    this.map.getSource('ai-route-source').setData({
                        type: 'Feature',
                        properties: {
                            ...metadata,
                            source: 'ai-route'
     ...
     */
    setAIWaypoints(waypoints, metadata) {
     ...
     */
    animateRoute(coordinates, options = {}) {
        ...
        const animate = (currentTime) => {
            ...
            if (this.map && this.map.getSource('planned-route-source')) {
                const partialCoordinates = coordinates.slice(0, Math.max(2, pointsToShow));
                this.map.getSource('planned-route-source').setData({
                    type: 'Feature',
                    properties: {
                        ...metadata,
    ...
    addWaypointPopups(waypoints) {
        ...
        this.map.on('mouseenter', 'ai-waypoints-layer', (e) => {
            if (!this.map)
                return;
    ...
    startGhostAnimation(ghost) {
        ...
        const ghostMarker = new window.mapboxgl.Marker({
    ...
    setPlannedRoute(geojson) {
        ...
        try {
            ...
            const lineFeature = featureCollection.features.find((f) => f.geometry.type === 'LineString');
            if (lineFeature) {
