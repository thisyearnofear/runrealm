...
import { getCurrentNetworkConfig, getContractConfig, getContractAddresses, isCorrectNetwork, CONTRACT_METHODS, CONTRACT_EVENTS, } from "../config/contracts";
export class ContractService extends BaseService {
    ...
    async initializeContracts() {
        try {
            ...
            this.universalContract = this.web3Service.getContract(universalConfig.address, universalConfig.abi);
            // Initialize Realm Token Contract
            this.realmTokenContract = this.web3Service.getContract(realmTokenConfig.address, realmTokenConfig.abi);
    ...
    setupEventListeners() {
        ...
        this.subscribe("web3:networkChanged", async (data) => {
            if (isCorrectNetwork(data.chainId)) {
                ...
                this.universalContract = null;
                this.realmTokenContract = null;
                console.warn(`Wrong network: ${data.chainId}. Expected: ${getCurrentNetworkConfig().chainId}`);
            }
        });
    }
    /**
     ...
     */
    async mintTerritory(territoryData) {
        ...
        if (!this.universalContract) {
            throw new Error("Universal contract not initialized. Please connect wallet and switch to ZetaChain.");
        }
        if (!this.web3Service.isConnected()) {
        ...
        try {
            ...
            const isClaimed = await this.universalContract[CONTRACT_METHODS.universal.isGeohashClaimed](territoryData.geohash);
            if (isClaimed) {
                throw new Error("Territory already claimed by another player");
            }
            // Estimate gas for the transaction
            const gasEstimate = await this.universalContract[CONTRACT_METHODS.universal.mintTerritory].estimateGas(territoryData.geohash, territoryData.difficulty, territoryData.distance, territoryData.landmarks);
            // Add 20% buffer to gas estimate
            const gasLimit = Math.ceil(Number(gasEstimate) * 1.2);
            // Execute the transaction
            const tx = await this.universalContract[CONTRACT_METHODS.universal.mintTerritory](territoryData.geohash, territoryData.difficulty, territoryData.distance, territoryData.landmarks, { gasLimit });
            ...
            if (receipt.status === 1) {
                ...
                try {
                    ...
                    const territoryCreatedEvent = receipt.logs.find((log) => {
                        try {
     ...
     */
    async handleCrossChainTerritoryClaim(crossChainData) {
        if (!this.universalContract) {
        ...
        try {
            ...
            // Check if geohash is already claimed
            const isClaimed = await this.universalContract[CONTRACT_METHODS.universal.isGeohashClaimed](crossChainData.geohash);
            if (isClaimed) {
                throw new Error("Territory already claimed by another player");
            }
            ...
            const tx = await this.universalContract[CONTRACT_METHODS.universal.mintTerritory](crossChainData.geohash, crossChainData.difficulty, crossChainData.distance, crossChainData.landmarks);
     ...
     */
    async processCrossChainMessage(context, zrc20, amount, message) {
        try {
     ...
     */
    async isGeohashClaimed(geohash) {
        if (!this.universalContract) {
            throw new Error("Universal contract not initialized");
        }
        try {
     ...
     */
    async getPlayerTerritories(address) {
        if (!this.universalContract) {
            throw new Error("Universal contract not initialized");
        }
        ...
        try {
            const tokenIds = await this.universalContract[CONTRACT_METHODS.universal.getPlayerTerritories](targetAddress);
     ...
     */
    async getTerritoryInfo(tokenId) {
        if (!this.universalContract) {
            throw new Error("Universal contract not initialized");
        }
        try {
            const info = await this.universalContract[CONTRACT_METHODS.universal.getTerritoryInfo](tokenId);
            return {
    ...
    async getPlayerStats(address) {
        ...
        try {
            const stats = await this.universalContract[CONTRACT_METHODS.universal.getPlayerStats](targetAddress);
            return {
     ...
     */
    async calculateTerritoryReward(difficulty, distance) {
        if (!this.universalContract) {
            throw new Error("Universal contract not initialized");
        }
        try {
            const reward = await this.universalContract[CONTRACT_METHODS.universal.calculateTerritoryReward](difficulty, distance);
     ...
     */
    async getRealmTokenBalance(address) {
        ...
        try {
            const balance = await this.realmTokenContract[CONTRACT_METHODS.realmToken.balanceOf](targetAddress);
