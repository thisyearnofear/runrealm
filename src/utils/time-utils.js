...
export function parsePace(text, useMetric) {
    if (!text)
        return null;
    ...
    if (timeUnit) {
        ...
        if (!unitToken)
            return seconds; // infer from useMetric outside
...
export function paceToSpeed(secondsPerUnit, useMetric) {
    // meters per second
    const metersPerUnit = useMetric ? 1000 : 1609.344;
    return metersPerUnit / secondsPerUnit;
}
export function etaFromDistance(distanceMeters, speedMps) {
