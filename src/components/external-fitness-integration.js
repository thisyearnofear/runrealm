...
export class ExternalFitnessIntegration extends BaseService {
    ...
    setupEventListeners() {
        const portal = this.container.querySelector('.fitness-portal');
        const closeBtn = portal.querySelector('.portal-close');
        const stravaCard = portal.querySelector('.service-card.strava');
        const loadMoreBtn = portal.querySelector('.load-more-btn');
    ...
    async connectStrava() {
        const stravaCard = this.container.querySelector('.service-card.strava');
        const status = stravaCard.querySelector('.service-status');
        status.textContent = 'Connecting...';
        stravaCard.classList.add('connecting');
        try {
            const authUrl = this.fitnessService.initiateStravaAuth();
            // Open OAuth window
            const authWindow = window.open(authUrl, 'strava-auth', 'width=600,height=600,scrollbars=yes,resizable=yes');
            if (!authWindow) {
    ...
    onConnectionFailed(source, error) {
        const serviceCard = this.container.querySelector(`.service-card.${source}`);
        const status = serviceCard.querySelector('.service-status');
        serviceCard.classList.remove('connecting');
        status.textContent = 'Failed';
        this.showError(`Failed to connect to ${source}: ${error}`);
    }
    onServiceConnected(source) {
        const serviceCard = this.container.querySelector(`.service-card.${source}`);
        const status = serviceCard.querySelector('.service-status');
        serviceCard.classList.remove('connecting');
    ...
    async loadRecentActivities(source, page) {
        try {
            if (source === 'strava') {
    ...
    displayActivities(activities) {
        const activitiesRealm = this.container.querySelector('.activities-realm');
        const grid = activitiesRealm.querySelector('.activities-grid');
    ...
    `).join('');
        ...
        grid.querySelectorAll('.preview-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const activityData = e.target.closest('.preview-btn')?.getAttribute('data-activity');
                if (activityData) {
        ...
        grid.querySelectorAll('.activity-card').forEach(card => {
            card.addEventListener('mouseover', async (e) => {
                const activityData = e.currentTarget.querySelector('.preview-btn')?.getAttribute('data-activity');
                if (activityData) {
    ...
    async previewTerritoryForActivity(activity) {
        const runTrackingService = this.getService('RunTrackingService');
        const mapService = this.getService('MapService');
        if (!runTrackingService || !mapService)
            return;
        const previewBtn = this.container.querySelector(`[data-activity-id="${activity.id}"] .preview-btn`);
     ...
     */
    showTerritoryClaimConfirmation(activity, territoryPreview) {
    ...
    `;
        ...
        confirmBtn?.addEventListener('click', async () => {
            try {
                const confirmBtnElement = confirmBtn;
                ...
                setTimeout(() => {
                    if (btnIcon)
                        btnIcon.textContent = '⚡';
                    if (btnText)
                        btnText.textContent = 'Claim Territory';
