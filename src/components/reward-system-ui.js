...
export class RewardSystemUI extends BaseService {
    ...
    showStakingModal() {
      ...
      `,
        ...
        const stakeInput = modal.querySelector('#stake-amount');
        ...
        stakeInput?.addEventListener('input', () => {
            ...
            if (dailyPreview)
                dailyPreview.textContent = `${this.formatTokenAmount(dailyReward)} REALM`;
            if (monthlyPreview)
    ...
    showRewardNotification(title, amount) {
     ...
     */
    addCelebrationEffects(widget, amount) {
    ...
    calculateRunReward(distance, duration) {
    ...
    addRewardStyles() {
        if (document.querySelector('#reward-system-styles'))
            return;
