...
export class AccessibilityEnhancer extends BaseService {
    ...
    handleEscapeKey() {
        ...
        if (onboardingOverlay) {
            const skipBtn = onboardingOverlay.querySelector('.tooltip-skip');
            skipBtn?.click();
            return;
        }
    ...
    navigateWithinWidget(currentElement, reverse) {
        const widget = currentElement.closest('.widget, .run-controls-widget');
        if (!widget)
            return;
        const focusableInWidget = widget.querySelectorAll('button:not([disabled]), input:not([disabled]), [tabindex]:not([tabindex="-1"])');
    ...
    addAccessibilityStyles() {
        if (document.querySelector('#accessibility-enhancer-styles'))
            return;
