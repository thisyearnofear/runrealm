...
export const RunRealmIntegration = ({ containerId = 'mapbox-container', gameMode = true, showWalletButton = true }) => {
    ...
    return (<div className="runrealm-integration">
      {/* Map container will be created programmatically */}
      
      {/* GameFi UI (Territory Dashboard) */}
      {isInitialized && gameMode && (<TerritoryDashboard initialPosition={currentPosition} gameMode={gameMode} onTerritorySelect={handleTerritorySelect} onRouteGenerated={handleRouteGenerated}/>)}

      {/* Wallet Connection Button */}
      {showWalletButton && !walletConnected && (<div className="wallet-connection-overlay">
      ...
      {selectedTerritory && (<div className="territory-modal-overlay" onClick={() => setSelectedTerritory(null)}>
          <div className="territory-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>🗺️ Territory Details</h3>
              <button className="close-modal" onClick={() => setSelectedTerritory(null)}>
                ×
              </button>
            </div>
            <div className="modal-content">
              <div className="territory-info">
                ...
                <div><strong>Difficulty:</strong> {selectedTerritory.difficulty < 33 ? 'Easy' :
                selectedTerritory.difficulty < 67 ? 'Medium' : 'Hard'}</div>
                ...
                {selectedTerritory.landmarks && selectedTerritory.landmarks.length > 0 && (<div>
                    <strong>Landmarks:</strong>
                    <ul>
                      {selectedTerritory.landmarks.map((landmark, i) => (<li key={i}>{landmark}</li>))}
                    </ul>
                  </div>)}
              </div>
            </div>
