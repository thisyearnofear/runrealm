...
export class WalletWidget extends BaseService {
    constructor(domService, uiService, animationService, web3Service) {
        super();
        this.walletState = { status: 'disconnected' };
        this.retryCount = 0;
        this.maxRetries = 3;
        ...
        this.walletProviders = [
            ...
            },
            {
                ...
                isInstalled: () => typeof window !== 'undefined' && Boolean(window.ethereum?.isCoinbaseWallet),
    ...
    setupEventListeners() {
        ...
        // Widget interactions
        this.domService.delegate(document.body, '[data-action="connect-wallet"]', 'click', () => {
            this.showWalletModal();
        });
        this.domService.delegate(document.body, '[data-action="disconnect-wallet"]', 'click', () => {
        ...
        this.domService.delegate(document.body, '.wallet-provider-option', 'click', (event) => {
            const providerId = event.target.closest('.wallet-provider-option')?.getAttribute('data-provider');
            if (providerId) {
    ...
    async checkExistingConnection() {
        try {
            ...
            if (isConnected && this.web3Service.isWalletAvailable()) {
                this.updateWalletState({ status: 'connecting', lastProvider: 'metamask' });
    ...
    async disconnectWallet() {
        try {
            ...
            this.uiService.showToast('Failed to disconnect wallet', { type: 'error' });
        }
    }
    showWalletModal() {
    ...
    hideWalletModal() {
        ...
        if (modal) {
            this.animationService.fadeOut(modal, { duration: 200 }).then(() => {
                modal.remove();
            });
        }
    }
    ...
    async retryConnection() {
        if (this.retryCount >= this.maxRetries) {
            this.uiService.showToast('Maximum retry attempts reached', { type: 'error' });
            return;
        }
        this.retryCount++;
    ...
    handleConnectionError(error, provider) {
    ...
    addWalletStyles() {
        if (document.querySelector('#wallet-widget-styles'))
            return;
