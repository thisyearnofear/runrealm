...
export class MobileWidgetService extends BaseService {
    constructor(touchGestureService, options = {}) {
        super();
    ...
    setupEdgeSwipe() {
        ...
        this.touchGestureService.addSwipeNavigation(document.body, () => {
     ...
     */
    applyMobileStyling(widgetElement) {
        if (!this.isMobile)
            return;
        // Add mobile class
     ...
     */
    removeMobileInappropriateContent(widgetElement) {
     ...
     */
    applyCompactLayoutOptimizations(widgetElement) {
     ...
     */
    consolidateWidgetContent(widgetElement) {
     ...
     */
    combineRelatedStats(statElements) {
     ...
     */
    createCompactStatDisplay(stats) {
        if (stats.length === 0)
            return;
     ...
     */
    minimizeWhitespace(widgetElement) {
     ...
     */
    optimizeButtonLayouts(widgetElement) {
     ...
     */
    constrainToViewport(widgetElement) {
        const widget = widgetElement.closest('.widget');
        if (!widget)
            return;
        ...
        const widgetZone = widget.closest('.widget-zone');
        if (widgetZone) {
        ...
        if (this.compactMode) {
            ...
            const nonEssential = widgetElement.querySelectorAll('.secondary-info, .help-text, .description, .widget-tip, .debug-info');
