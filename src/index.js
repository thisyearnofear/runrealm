...
if (process.env.NODE_ENV === 'production') {
    ...
    console.error = (...args) => {
        ...
        if (containsToken(message)) {
            const sanitizedArgs = args.map(arg => typeof arg === 'string' ? sanitizeMessage(arg) : arg);
...
async function initializeApp() {
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            window.RunRealm = app; // Also expose as RunRealm for consistency
            // Expose widget system debug utilities
            window.debugWidgets = () => {
                const mainUI = app.mainUI;
